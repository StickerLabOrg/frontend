...
function getFaixaClass(posicao: number) {
  if (posicao <= 4) return "bg-emerald-900/40";
  if (posicao <= 6) return "bg-sky-900/40";
  if (posicao >= 17) return "bg-red-900/40";
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto w-full">
        ...
        <p className="text-mutedForeground mb-6">Tabela oficial da temporada.</p>
          ...
          <CardContent className="p-6">
                  ...
                  <tbody>
                    {tabela.map((row, index) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-2 px-3 text-gray-300">{row.posicao}</td>

                          {/* ITEM DO TIME */}
                          <td className="py-2 px-3 flex items-center gap-3">

                            {/* ⭐ BOLINHA COLORIDA DISCRETA (TAMANHO CERTO) ⭐ */}
                            <div
                              className={`
                                h-6 w-6 rounded-full border border-white/20 shadow-sm 
                                ${TEAM_COLORS[row.time_nome] ?? "bg-white/20"}
                              `}
                            />

                            <span className="font-medium">{row.time_nome}</span>
                          </td>

                          <td className="py-2 px-2 text-center font-bold">{row.pontos}</td>
                          <td className="py-2 px-2 text-center">{row.jogos}</td>
                          <td className="py-2 px-2 text-center">{row.vitorias}</td>
                          <td className="py-2 px-2 text-center">{row.empates}</td>
                          <td className="py-2 px-2 text-center">{row.derrotas}</td>
                          <td className="py-2 px-2 text-center">{row.gols_pro}</td>
                          <td className="py-2 px-2 text-center">{row.gols_contra}</td>

                          <td className={`py-2 px-2 text-center ${saldoClass}`}>
            ...
            <div className="mt-5 flex gap-4 text-xs text-gray-300">
              <div className="flex items-center gap-2">
                <span className="w-6 h-3 bg-emerald-700 rounded" /> Libertadores (G4)
              </div>
              <div className="flex items-center gap-2">
                <span className="w-6 h-3 bg-sky-700 rounded" /> Pré-Libertadores (5–6)
              </div>
              <div className="flex items-center gap-2">
                <span className="w-6 h-3 bg-red-800 rounded" /> Rebaixamento (Z4)
              </div>
            </div>
