...
export function PartidaPage() {
  ...
  async function fetchPartidaDetalhes(id: string) {
    try {
      ...
      // 🔥 ROTA CORRETA DA API: /partidas/resultado/{event_id}
      const resp = await axios.get(
        `http://localhost:8000/partidas/resultado/${id}`
      );

      // A API pode devolver em formatos diferentes, então tratamos todos
      ...
      const detalhe: PartidaDetalhe = {
        id_partida: String(data.id_partida ?? data.idEvent ?? id),
        time_casa: {
          nome:
            data.time_casa?.nome ??
            data.time_casa_nome ??
          ...
          escudo:
            ...
            data.strHomeBadge ??
            null,
          id: data.time_casa?.id ?? data.time_casa_id ?? data.idHomeTeam,
        },
        time_fora: {
          nome:
            data.time_fora?.nome ??
          ...
          escudo:
            ...
            data.strAwayBadge ??
            null,
          id: data.time_fora?.id ?? data.time_fora_id ?? data.idAwayTeam,
        },
        data: data.data ?? data.data_evento ?? data.dateEvent ?? "",
        horario: data.horario ?? data.hora_evento ?? data.strTime ?? "",
        status: data.status ?? data.strStatus ?? null,
        placar_casa:
          data.placar_casa ??
          data.gols_casa ??
          (data.intHomeScore != null
            ? Number(data.intHomeScore)
            : null),
        placar_fora:
          data.placar_fora ??
          data.gols_fora ??
          (data.intAwayScore != null
            ? Number(data.intAwayScore)
            : null),
  ...
  useEffect(() => {
    if (partidaId) {
      fetchPartidaDetalhes(partidaId);
    } else {
      setLoading(false);
              ...
              <div className="flex flex-col gap-6 md:gap-8">
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-10 md:gap-16">
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div className="h-16 w-16 rounded-full bg-white-500 shadow-lg shadow-white-500/40 overflow-hidden flex items-center justify-center">
          ...
          <div className="inline-flex bg-slate-900/80 border border-white/10 rounded-full p-1">
            {[
              ...
              <button
                ...
                className={`
                  px-4 py-1 text-xs md:text-sm rounded-full transition-all
                  ${
                    abaAtiva === aba.id
          ...
          <CardContent className="p-5 md:p-6">
            {abaAtiva === "resumo" && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-300">
                  Em breve você poderá acompanhar aqui a timeline completa da
                  partida, com gols, cartões, substituições e lances importantes
                  em tempo real, usando os dados da TerraSportDB.
                </p>
              </div>
            )}
            ...
            {abaAtiva === "estatisticas" && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-300">
                  As estatísticas detalhadas (posse de bola, finalizações,
                  passes, etc.) serão integradas assim que a API retornar esses
                  dados para a partida.
                </p>
              </div>
            )}
            ...
            {abaAtiva === "escalacoes" && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-300">
                  Em breve você verá aqui as escalações completas dos times,
                  com titulares, reservas e esquema tático.
                </p>
              </div>
            )}
          </CardContent>
